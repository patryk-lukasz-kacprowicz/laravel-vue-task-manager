                            ...
                            <template v-for="item in navigation" :key="item.name">
                                ...
                                <Menu v-else as="div" class="relative">
                                    ...
                                    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
                                        <MenuItems class="absolute left-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-neutral-900 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                            <template v-for="child in item.children" :key="child.name || 'separator'">
                                                ...
                                                <MenuItem v-else v-slot="{ active }">
                                                    <a :href="child.href" :class="[active ? 'bg-neutral-900' : '', 'block px-4 py-2 text-sm text-gray-400']">{{ child.name }}</a>
                                                </MenuItem>
                                            </template>
                                        </MenuItems>
                                    </transition>
